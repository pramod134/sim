...
def evaluate_bos_fvg_ltf(
    ...
    def _close_trade(
        ...
        trade = {
            ...
            "breakeven_price": p.get("be_runner_sl_price") if p.get("be_runner_sl_price") is not None else p.get("breakeven_price"),
    ...
    if pos and close_px is not None:
        ...
        if not pos.get("tp1_hit") and _safe_int(pos.get("total_shares_open"), 0) > 0:
            ...
            if tp1_hit:
                ...
                pnl_partial = _close_partial_position(tp1_exit_shares, tp1_price)
                ...
                bos1_runner = remaining_shares - (remaining_shares // 2)
                bos2_runner = remaining_shares // 2

                pos["bos1_runner_shares"] = bos1_runner
                pos["bos2_runner_shares"] = bos2_runner
                pos["be_runner_active"] = bos2_runner > 0
                pos["be_runner_sl_price"] = avg_entry if bos2_runner > 0 else None
                pos["be_runner_armed_ts"] = last_ts if bos2_runner > 0 else None
                pos["breakeven_price"] = avg_entry if bos2_runner > 0 else None
                ...
                print(
                    ...
                    f"RemainingSize={remaining_shares} | BOS1RunnerShares={bos1_runner} | BOS2RunnerShares={bos2_runner} | "
            ...
            if is_opposite:
                ...
                if bos_key not in consumed:
                    ...
                    if not pos.get("tp1_hit") or shares_open <= 0:
                        ...
                        if exit_shares > 0:
                            pnl_partial = _close_partial_position(exit_shares, close_px)
        ...
        pos = state.get("open_position")
        # BE SL is CLOSE-BASED and applies only to the BOS2 runner
        if pos and bool(pos.get("be_runner_active")) and _safe_int(pos.get("bos2_runner_shares"), 0) > 0:
            ...
            if active_sl is not None:
                ...
                if not be_same_candle_as_arm:
                    be_hit = (side == "long" and close_px is not None and close_px <= active_sl) or (side == "short" and close_px is not None and close_px >= active_sl)
                if be_hit:
                    exit_shares = _safe_int(pos.get("bos2_runner_shares"), 0)
                    if DEBUG_LOGS:
                        print(
                    ...
                    if exit_shares > 0:
                        pnl_partial = _close_partial_position(exit_shares, active_sl)
                        pos["be_runner_active"] = False
                        pos["bos2_runner_shares"] = 0
                        pos["partial_exit_count"] = _safe_int(pos.get("partial_exit_count"), 0) + 1
                        print(
