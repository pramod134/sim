...
async def main() -> int:
    ...
    _sb_env()
    print("[SIM_WORKER] Starting worker loop")
    ...
    async with httpx.AsyncClient() as client:
        ...
        if run_id:
            try:
                ...
                print(f"[SIM_WORKER] simulation_runs row created id={run_id} symbol={symbol}")
            except Exception as e:
                sim_run_logging_enabled = False
                print(f"[SIM_WORKER][WARN] Failed to create simulation_runs row id={run_id}: {type(e).__name__}: {e}")
        else:
            print("[SIM_WORKER][WARN] Missing run_id in claimed job; simulation_runs logging disabled")
        ...
        print(f"[SIM_WORKER] Claimed job: symbol={symbol} seed_date={seed_date} sim_period={sim_period} run_id={run_id}")
        ...
        try:
            ...
            print(f"[SIM_WORKER] Sim days count={len(sim_days)} preview={sim_days[:3]}{'...' if len(sim_days) > 3 else ''}")
                ...
                async for event in engine.stream_day(symbol=symbol, date_et=d):
                    ...
                    if sim_run_logging_enabled and emitted_events % 250 == 0:
                        try:
                            ...
                            print(f"[SIM_WORKER][WARN] periodic simulation_runs update failed id={run_id}: {type(e).__name__}: {e}")
            ...
            if sim_run_logging_enabled:
                try:
                    ...
                    print(f"[SIM_WORKER] simulation_runs completed id={run_id} events={emitted_events} trades={len(trades)}")
                except Exception as e:
                    print(f"[SIM_WORKER][WARN] final simulation_runs update failed id={run_id}: {type(e).__name__}: {e}")
            ...
            try:
                if sim_run_logging_enabled and run_id:
                    ...
                    print(f"[SIM_WORKER] simulation_runs failed id={run_id}")
                await _mark_error(client, symbol, msg)
            except Exception as e2:
                print(f"[SIM_WORKER] Failed to mark error in DB: {type(e2).__name__}: {e2}")
            return 1
